        ...
        <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {BENEFITS.map((benefit) => (
            <li key={benefit.title} className="flex gap-4">
              <span className="grid size-11 shrink-0 place-items-center rounded-full bg-white text-denim-700 shadow-sm">
                <benefit.icon className="size-5" />
              </span>
              <div>
                <h3 className="text-sm font-semibold text-ink-900">{benefit.title}</h3>
                <p className="mt-1 text-sm leading-relaxed text-ink-500">{benefit.description}</p>
              </div>
            </li>
          ))}
        </ul>
